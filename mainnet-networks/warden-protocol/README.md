---
description: >-
  Warden Protocol is an intent-centric protocol, built on the Cosmos SDK, that
  enables various execution layers for interoperability
---

# Warden Protocol

### Explorer

{% embed url="https://explorer.aknodes.com/WARDEN-TESTNET/" %}



<figure><img src="../../.gitbook/assets/warden.webp" alt=""><figcaption></figcaption></figure>

<table><thead><tr><th>Chain ID</th><th width="218.33333333333331">Version tag</th></tr></thead><tbody><tr><td>warden_8765-1</td><td>v0.7.2</td></tr></tbody></table>



| Binary Name | Wasm    | SDK version |
| ----------- | ------- | ----------- |
| wardend     | Enabled | v0.50.5     |

{% tabs %}
{% tab title="RPC" %}
https://rpc.warden-mainnet.aknodes.net
{% endtab %}

{% tab title="API" %}
https://api.warden-mainnet.aknodes.net
{% endtab %}

{% tab title="gRPC" %}
grpc.warden.aknodes.net:18090
{% endtab %}
{% endtabs %}
